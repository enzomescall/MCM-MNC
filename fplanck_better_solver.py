...
class fokker_planck:
    def __init__(self, *, temperature, drag, extent, resolution,
        ...
        # drag is diffusion, more or less
        # acceleration is the force, more or less

        # encapsulates the extent array into an array and then sets the ndim value
        self.extent = np.atleast_1d(extent)
        self.ndim = self.extent.size

        # sets the temperature and resolution variables
        self.temperature = value_to_vector(temperature, self.ndim)
        self.resolution  = value_to_vector(resolution, self.ndim)

        # sets the other initial variables
        self.potential = potential
        self.force = force
        self.boundary = value_to_vector(boundary, self.ndim, dtype=object)

        # sets a value for the constant beta (TODO FIGURE THIS OUT)
        ...
        # Here, we center the axis at 0
        for axis in axes:
            axis -= np.average(axis)
        self.axes = axes
        # Finally, we make the grid itself, as a mesh grid
        ...
        # If callable, set self.drag to values evaluated at grid
        if callable(drag):
            self.drag[...] = drag(*self.grid)
        # otherwise, set with simple evaluation
        elif np.isscalar(drag):
            self.drag[...] = drag
        # finally if a vector (iterable), set informed on that.
        ...
        # Does this odd thing where the mobility is 1/drag for some reason, as is diffusion. Maybe
        # we can comment it out for later (TODO FIGURE THIS OUT)
        self.mobility = 1/self.drag
        for i in range(self.ndim):
            self.diffusion[i] = constants.k*self.temperature[i]/self.drag[i]

        if self.potential is not None:
            # evaluate potential at grid and set U from that
            U = self.potential(*self.grid)
            # set potential values to the values of U
            self.potential_values += U
            # force values are spaced out based on resolution, and they are negative (TODO WHY NEGATIVE?)
